...
`;
  ...
  const apiUrl = `https://api.mindmaid.ai/api/embed/bot/${id}`;
  async function fetchData() {
    try {
      ...
      logo_greeting = data.logo_greeting;
      iconLogo = data.preview_logo;
      boxChat_colorWidgetBg = data.boxChat_colorWidgetBg;
  ...
  async function CheckPublicBot() {
    ...
    if (check === true) {
      // let i = `http://localhost:3003/embed/${id ? id : null}`,
      let i = `https://mindmaid.ai/embed/${id ? id : null}`,
      ...
      greetingText.textContent = logo_greeting ? logo_greeting : "";
  ...
  <circle cx="19.8961" cy="24.8046" r="4.36782" fill="white"/>
      ...
      // iconHand.style.width = "22px";
      // iconHand.style.height = "22px";
      // iconHand.style.marginLeft = "4px";
